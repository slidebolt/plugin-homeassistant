...
func TestToHA_LightSupportedModesComeFromCommands(t *testing.T) {
	entity := domain.Entity{
		Type:     "light",
		Commands: []string{"light_turn_on", "light_turn_off", "light_set_brightness", "light_set_color_temp", "light_set_hs"},
		State:    domain.Light{Power: false, Brightness: 0},
	}
	state, _ := ToHA(entity)
	modes, ok := state["supported_color_modes"].([]string)
	if !ok {
		t.Fatalf("supported_color_modes type: %T", state["supported_color_modes"])
	}
	...
	if !found["color_temp"] {
		t.Fatalf("missing color_temp mode: %v", modes)
	}
	if !found["hs"] {
		t.Fatalf("missing hs mode: %v", modes)
	}
}
