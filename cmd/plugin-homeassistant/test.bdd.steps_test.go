...
type bddCtx struct {
	...
	lastHAState   map[string]any
	lastHAAttrs   map[string]any
	lastFromHACmd any
	lastFromHAErr error
}
...
func (c *bddCtx) climateTemperatureIs(expected int) error {
	...
	if st.Temperature != float64(expected) {
		return fmt.Errorf("climate.Temperature: got %v, want %v", st.Temperature, float64(expected))
	}
	return nil
}
...
func makeCommand(action string) (messenger.Action, error) {
	...
	case "camera_enable_motion":
		return domain.CameraEnableMotion{}, nil
	case "camera_disable_motion":
		return domain.CameraDisableMotion{}, nil
	case "valve_open":
		return domain.ValveOpen{}, nil
	case "valve_close":
		return domain.ValveClose{}, nil
	case "valve_set_position":
		return domain.ValveSetPosition{Position: 50}, nil
	case "siren_turn_on":
		return domain.SirenTurnOn{}, nil
	case "siren_turn_off":
		return domain.SirenTurnOff{}, nil
	case "siren_set_tone":
		return domain.SirenSetTone{Tone: "alert"}, nil
	case "humidifier_turn_on":
		return domain.HumidifierTurnOn{}, nil
	case "humidifier_turn_off":
		return domain.HumidifierTurnOff{}, nil
	case "humidifier_set_humidity":
		return domain.HumidifierSetHumidity{Humidity: 50}, nil
	case "humidifier_set_mode":
		return domain.HumidifierSetMode{Mode: "auto"}, nil
	case "media_play":
	...
	case "media_set_volume":
		return domain.MediaSetVolume{VolumeLevel: 0.5}, nil
	case "media_mute":
		return domain.MediaMute{Mute: true}, nil
	case "media_select_source":
		return domain.MediaSelectSource{Source: "radio"}, nil
	case "remote_turn_on":
		return domain.RemoteTurnOn{}, nil
	case "remote_turn_off":
		return domain.RemoteTurnOff{}, nil
	case "remote_set_activity":
		return domain.RemoteSetActivity{Activity: "watch_tv"}, nil
	case "remote_send_command":
		return domain.RemoteSendCommand{Command: "power"}, nil
	default:
...
// ---------------------------------------------------------------------------
// New entity types: creation steps
// ---------------------------------------------------------------------------

func (c *bddCtx) anAlarmEntity(key, name, alarmState string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "alarm", Name: name,
		State: domain.Alarm{AlarmState: alarmState},
	})
}

func (c *bddCtx) aCameraEntityNotStreaming(key, name string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "camera", Name: name,
		State: domain.Camera{IsStreaming: false},
	})
}

func (c *bddCtx) aValveEntityAtPosition(key, name string, position int) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "valve", Name: name,
		State: domain.Valve{Position: position},
	})
}

func (c *bddCtx) aSirenEntityWithPower(key, name, powerStr string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "siren", Name: name,
		State: domain.Siren{IsOn: powerStr == "on"},
	})
}

func (c *bddCtx) aHumidifierEntity(key, name string, targetHumidity int) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "humidifier", Name: name,
		State: domain.Humidifier{TargetHumidity: targetHumidity},
	})
}

func (c *bddCtx) aMediaPlayerEntityWithState(key, name, state string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "media_player", Name: name,
		State: domain.MediaPlayer{State: state},
	})
}

func (c *bddCtx) aRemoteEntityWithPower(key, name, powerStr string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		ID: id, Plugin: plug, DeviceID: dev,
		Type: "remote", Name: name,
		State: domain.Remote{IsOn: powerStr == "on"},
	})
}

func (c *bddCtx) anEventEntity(key, name, deviceClass string) error {
	plug, dev, id, err := parseKey(key)
	if err != nil {
		return err
	}
	return c.saveEntity(domain.Entity{
		...
		State: domain.Event{DeviceClass: deviceClass},
	})
}

// ---------------------------------------------------------------------------
// New entity types: update steps
// ---------------------------------------------------------------------------

func (c *bddCtx) iUpdateAlarmState(key, alarmState string) error {
	entity, err := c.getEntity(key)
	if err != nil {
		return err
	}
	...
	entity.State = s
	return c.saveEntity(entity)
}

// ---------------------------------------------------------------------------
// New entity types: assertion steps
// ---------------------------------------------------------------------------

func (c *bddCtx) theAlarmStateIs(expected string) error {
	s, ok := c.lastEntity.State.(domain.Alarm)
	if !ok {
		return fmt.Errorf("entity state is not Alarm, got %T", c.lastEntity.State)
	}
	if s.AlarmState != expected {
		return fmt.Errorf("alarm state: got %q, want %q", s.AlarmState, expected)
	}
	return nil
}

func (c *bddCtx) theCameraIsNotStreaming() error {
	s, ok := c.lastEntity.State.(domain.Camera)
	if !ok {
		return fmt.Errorf("entity state is not Camera, got %T", c.lastEntity.State)
	}
	if s.IsStreaming {
		return fmt.Errorf("camera is streaming, want not streaming")
	}
	return nil
}

func (c *bddCtx) theValvePositionIs(expected int) error {
	s, ok := c.lastEntity.State.(domain.Valve)
	if !ok {
		return fmt.Errorf("entity state is not Valve, got %T", c.lastEntity.State)
	}
	if s.Position != expected {
		return fmt.Errorf("valve position: got %d, want %d", s.Position, expected)
	}
	return nil
}

func (c *bddCtx) theSirenIsOff() error {
	s, ok := c.lastEntity.State.(domain.Siren)
	if !ok {
		return fmt.Errorf("entity state is not Siren, got %T", c.lastEntity.State)
	}
	if s.IsOn {
		return fmt.Errorf("siren is on, want off")
	}
	return nil
}

func (c *bddCtx) theHumidifierTargetHumidityIs(expected int) error {
	s, ok := c.lastEntity.State.(domain.Humidifier)
	if !ok {
		return fmt.Errorf("entity state is not Humidifier, got %T", c.lastEntity.State)
	}
	if s.TargetHumidity != expected {
		return fmt.Errorf("target humidity: got %d, want %d", s.TargetHumidity, expected)
	}
	return nil
}

func (c *bddCtx) theMediaPlayerStateIs(expected string) error {
	s, ok := c.lastEntity.State.(domain.MediaPlayer)
	if !ok {
		return fmt.Errorf("entity state is not MediaPlayer, got %T", c.lastEntity.State)
	}
	if s.State != expected {
		return fmt.Errorf("media player state: got %q, want %q", s.State, expected)
	}
	return nil
}

func (c *bddCtx) theRemoteIsOn() error {
	s, ok := c.lastEntity.State.(domain.Remote)
	if !ok {
		return fmt.Errorf("entity state is not Remote, got %T", c.lastEntity.State)
	}
	if !s.IsOn {
		return fmt.Errorf("remote is off, want on")
	}
	return nil
}

func (c *bddCtx) theEventDeviceClassIs(expected string) error {
	s, ok := c.lastEntity.State.(domain.Event)
	if !ok {
		return fmt.Errorf("entity state is not Event, got %T", c.lastEntity.State)
	}
	if s.DeviceClass != expected {
		return fmt.Errorf("event device class: got %q, want %q", s.DeviceClass, expected)
	}
	return nil
}

// ---------------------------------------------------------------------------
// ToHA translation steps
// ---------------------------------------------------------------------------

func (c *bddCtx) iTranslateEntityToHAWireFormat(key string) error {
	entity, err := c.getEntity(key)
	if err != nil {
		return fmt.Errorf("getEntity %q: %w", key, err)
	}
	c.lastHAState, c.lastHAAttrs = translate.ToHA(entity)
	return nil
}

func (c *bddCtx) theHAWireStateFieldIsTrue(field string) error {
	v, ok := c.lastHAState[field]
	if !ok {
		return fmt.Errorf("HA state missing field %q; state=%v", field, c.lastHAState)
	}
	b, ok := v.(bool)
	if !ok || !b {
		return fmt.Errorf("HA state field %q: got %v (%T), want true", field, v, v)
	}
	return nil
}

func (c *bddCtx) theHAWireStateFieldIsFalse(field string) error {
	v, ok := c.lastHAState[field]
	if !ok {
		return fmt.Errorf("HA state missing field %q; state=%v", field, c.lastHAState)
	}
	b, ok := v.(bool)
	if !ok || b {
		return fmt.Errorf("HA state field %q: got %v (%T), want false", field, v, v)
	}
	return nil
}

func (c *bddCtx) theHAWireStateFieldEqualsInt(field string, expected int) error {
	v, ok := c.lastHAState[field]
	if !ok {
		return fmt.Errorf("HA state missing field %q; state=%v", field, c.lastHAState)
	}
	...
	case float64:
		got = int(n)
	default:
		return fmt.Errorf("HA state field %q: got %v (%T), want int", field, v, v)
	}
	if got != expected {
		return fmt.Errorf("HA state field %q: got %d, want %d", field, got, expected)
	}
	return nil
}

func (c *bddCtx) theHAWireStateFieldEqualsString(field, expected string) error {
	v, ok := c.lastHAState[field]
	if !ok {
		return fmt.Errorf("HA state missing field %q; state=%v", field, c.lastHAState)
	}
	got := fmt.Sprintf("%v", v)
	if got != expected {
		return fmt.Errorf("HA state field %q: got %q, want %q", field, got, expected)
	}
	return nil
}
...
func (c *bddCtx) hASendsActionToEntityTypeWithNoParams(action, entityType string) error {
	c.lastFromHACmd, c.lastFromHAErr = translate.FromHA(entityType, action, map[string]any{})
	return nil
}

func (c *bddCtx) hASendsActionToEntityTypeWithParams(action, entityType, paramsJSON string) error {
	var params map[string]any
	if err := json.Unmarshal([]byte(paramsJSON), &params); err != nil {
		return fmt.Errorf("parse params: %w", err)
	}
	c.lastFromHACmd, c.lastFromHAErr = translate.FromHA(entityType, action, params)
	return nil
}

func (c *bddCtx) theDomainCommandTypeIs(expected string) error {
	if c.lastFromHAErr != nil {
		return fmt.Errorf("FromHA returned error: %v", c.lastFromHAErr)
	}
	got := reflect.TypeOf(c.lastFromHACmd).Name()
	if got != expected {
		return fmt.Errorf("domain command type: got %q, want %q", got, expected)
	}
	return nil
}

func (c *bddCtx) theDomainCommandFieldEqualsInt(field string, expected int) error {
	if c.lastFromHAErr != nil {
		return fmt.Errorf("FromHA returned error: %v", c.lastFromHAErr)
	}
	v := reflect.ValueOf(c.lastFromHACmd)
	f := v.FieldByName(field)
	if !f.IsValid() {
		return fmt.Errorf("command %T has no field %q", c.lastFromHACmd, field)
	}
	var got int
	switch f.Kind() {
	...
	case reflect.Float32, reflect.Float64:
		got = int(f.Float())
	default:
		return fmt.Errorf("field %q is %v, not int", field, f.Kind())
	}
	if got != expected {
		return fmt.Errorf("field %q: got %d, want %d", field, got, expected)
	}
	return nil
}
...
func (c *bddCtx) RegisterSteps(ctx *godog.ScenarioContext) {
	...
	// --- New entity types: creation ---
	ctx.Step(`^an alarm entity "([^"]*)" named "([^"]*)" with state "([^"]*)"$`, c.anAlarmEntity)
	ctx.Step(`^a camera entity "([^"]*)" named "([^"]*)" not streaming$`, c.aCameraEntityNotStreaming)
	ctx.Step(`^a valve entity "([^"]*)" named "([^"]*)" at position (\d+)$`, c.aValveEntityAtPosition)
	ctx.Step(`^a siren entity "([^"]*)" named "([^"]*)" with power (on|off)$`, c.aSirenEntityWithPower)
	ctx.Step(`^a humidifier entity "([^"]*)" named "([^"]*)" with target humidity (\d+)$`, c.aHumidifierEntity)
	ctx.Step(`^a media_player entity "([^"]*)" named "([^"]*)" with state "([^"]*)"$`, c.aMediaPlayerEntityWithState)
	ctx.Step(`^a remote entity "([^"]*)" named "([^"]*)" with power (on|off)$`, c.aRemoteEntityWithPower)
	ctx.Step(`^an event entity "([^"]*)" named "([^"]*)" with device class "([^"]*)"$`, c.anEventEntity)

	// --- New entity types: update ---
	ctx.Step(`^I update "([^"]*)" alarm state to "([^"]*)"$`, c.iUpdateAlarmState)

	// --- New entity types: assertions ---
	ctx.Step(`^the alarm state is "([^"]*)"$`, c.theAlarmStateIs)
	ctx.Step(`^the camera is not streaming$`, c.theCameraIsNotStreaming)
	ctx.Step(`^the valve position is (\d+)$`, c.theValvePositionIs)
	ctx.Step(`^the siren is off$`, c.theSirenIsOff)
	ctx.Step(`^the humidifier target humidity is (\d+)$`, c.theHumidifierTargetHumidityIs)
	ctx.Step(`^the media player state is "([^"]*)"$`, c.theMediaPlayerStateIs)
	ctx.Step(`^the remote is on$`, c.theRemoteIsOn)
	ctx.Step(`^the event device class is "([^"]*)"$`, c.theEventDeviceClassIs)

	// --- ToHA steps ---
	ctx.Step(`^I translate "([^"]*)" to HA wire format$`, c.iTranslateEntityToHAWireFormat)
	ctx.Step(`^the HA wire state field "([^"]*)" is true$`, c.theHAWireStateFieldIsTrue)
	ctx.Step(`^the HA wire state field "([^"]*)" is false$`, c.theHAWireStateFieldIsFalse)
	ctx.Step(`^the HA wire state field "([^"]*)" equals (\d+)$`, c.theHAWireStateFieldEqualsInt)
	ctx.Step(`^the HA wire state field "([^"]*)" equals "([^"]*)"$`, c.theHAWireStateFieldEqualsString)

	// --- FromHA steps ---
	ctx.Step(`^HA sends "([^"]*)" to entity type "([^"]*)" with no params$`, c.hASendsActionToEntityTypeWithNoParams)
	ctx.Step(`^HA sends "([^"]*)" to entity type "([^"]*)" with params '([^']*)'$`, c.hASendsActionToEntityTypeWithParams)
	ctx.Step(`^the domain command type is "([^"]*)"$`, c.theDomainCommandTypeIs)
	ctx.Step(`^the domain command field "([^"]*)" equals (\d+)$`, c.theDomainCommandFieldEqualsInt)
}
