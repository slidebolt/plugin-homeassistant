Feature: Light Entity
  ...
  Scenario: ToHA wire state has correct fields
    Given a light entity "test.dev1.toha" named "Test Light" with power on brightness 200 temperature 0
    When I translate "test.dev1.toha" to HA wire format
    Then the HA wire state field "is_on" is true
    And the HA wire state field "brightness" equals 200

  Scenario: FromHA turn_on maps to domain command
    When HA sends "turn_on" to entity type "light" with no params
    Then the domain command type is "LightTurnOn"

  Scenario: FromHA turn_on with brightness maps to domain command
    When HA sends "turn_on" to entity type "light" with params '{"brightness": 200}'
    Then the domain command type is "LightSetBrightness"
    And the domain command field "Brightness" equals 200
