Feature: Climate Entity
  ...
  Scenario: ToHA wire state has correct fields
    Given a climate entity "test.dev1.toha" named "Test Climate" with hvac_mode "cool" temperature 22
    When I translate "test.dev1.toha" to HA wire format
    Then the HA wire state field "hvac_mode" equals "cool"

  Scenario: FromHA set_hvac_mode maps to domain command
    When HA sends "set_hvac_mode" to entity type "climate" with params '{"hvac_mode": "cool"}'
    Then the domain command type is "ClimateSetMode"
